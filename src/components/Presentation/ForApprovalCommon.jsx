...
const ForApprovalCommon = ({ changeRequests }) => {
    ...
    useEffect(() => {
        ...
        const filteredRequests = changeRequests?.toApprove?.filter(request =>
            request.change_sites && request.change_sites.split(',').length > 1 // Added check for request.change_sites existence
        ) || [];
    ...
    const handleApprovalChange = async (requestId, newValue) => {
        ...
        try {
            const response = await axiosPrivate.put('/change-requests/update-approval', {
                id: requestId,
                approval: newValue // Send the new value (e.g., "YES", "NO", "Waiting")
            });
            ...
            setApprovals(prevApprovals => {
                 const valueBeforeOptimisticUpdate = approvals[requestId];
                 return {
                     ...prevApprovals,
                     [requestId]: valueBeforeOptimisticUpdate // Revert
                 };
            });
        }
    };
    ...
    const filteredChangeRequests = changeRequests?.toApprove?.filter(request =>
        request.change_sites && request.change_sites.split(',').length > 1
    ) || [];
                        ...
                        {filteredChangeRequests.map((request, index) => ( // Keep index for the tr key
                            <tr key={index}> {/* Using index for the React list key is fine */}
                                ...
                                <td className="p-2 border border-gray-300 text-center align-top">
                                    ...
                                    >
                                        {/* Optional: Add a disabled placeholder if the initial value isn't YES/NO/Waiting */}
                                        {/* This maps 'Waiting' backend value to 'HOLD' UI label */}
                                        ...
                                        <option value="Waiting">HOLD</option> {/* Map UI "HOLD" to backend "Waiting" */}
                                    </select>
                                </td>
                            </tr>))}
                    </tbody>
