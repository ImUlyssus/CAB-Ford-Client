...
const ForApprovalFTM = ({changeRequests}) => {
    return (
        <div className="w-full h-full bg-white p-8">
            {/* Title */}
            <h1 className="text-2xl font-bold mb-6 text-[#003478] border-b-2 border-gray-300 pb-2">
                <span className='text-red-500'>FTM</span> - Change Requests for Approval
            </h1>
            ...
            <div className="w-full max-h-[80%] overflow-y-auto border border-gray-300">
                <table className="min-w-full border border-gray-300">
                    ...
                    <tbody className="text-black text-xs">
                        {changeRequests.toApprove.filter(request => request.change_sites.includes('ftm') && !request.change_sites.includes('aat') && !request.change_sites.includes('fsst')).map((request, index) => (
                            <tr key={index}>
